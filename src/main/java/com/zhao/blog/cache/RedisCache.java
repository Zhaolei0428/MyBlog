package com.zhao.blog.cache;

import com.fasterxml.jackson.databind.JavaType;
import com.fasterxml.jackson.databind.ObjectMapper;
import com.zhao.blog.enums.RedisKeyEnum;
...
public class RedisCache {
    ...
    public long getBlogNum() {
        ...
        try {
            ...
            if(blogSet == null || blogSet.isEmpty()){
                ...
                List<Long> allBlogId = blogService.getAllBlogId(null);
                logger.info(allBlogId.toString());
                System.out.println(allBlogId);
                if(allBlogId == null || allBlogId.isEmpty())return blogNum;
                int size = allBlogId.size();
                for(int i = 0; i < size; i++){
                    Blog blog = blogService.getBlogByBlogId(allBlogId.get(i));
                    int num = blog.getBrowse();
                    jedis.zadd(blogScoreKey, num, allBlogId.get(i).toString());
                    blogNum += num;
                }
    ...
    public long getBlogNumById(String blogId){
        ...
        long blogNum = 0;
        try {
            blogNum = jedis.zincrby(blogScoreKey, 1, blogId).longValue();
//            System.out.println(blogNum);
        ...
        try {
            ...
            for(Tuple tuple: blogSet){
                ...
                if(StringUtils.isEmpty(blogInfo)){
                    blog = blogService.getBlogByBlogId(Long.parseLong(blogId));
     ...
     * @return
     */
    public Blog getBlogInfo(String blogId){
...
//            System.out.println(start + " " + end + blogIdList);
            if(blogIdList == null || blogIdList.isEmpty()){
                List<Long> allBlogId = blogService.getAllBlogId(blogType);
                if(allBlogId == null || allBlogId.isEmpty()) return new ArrayList<>();
                String[] allBlogArray = new String[allBlogId.size()];
                for(int i=0;i<allBlogId.size();i++)
                    allBlogArray[i] = String.valueOf(allBlogId.get(i));
    ...
    public List<HashMap<String, Object>> getBlogInfoList(List<String> blogIdList){
        ...
        try {
            String[] blogIdArray = blogIdList.toArray(new String[blogIdList.size()]);
